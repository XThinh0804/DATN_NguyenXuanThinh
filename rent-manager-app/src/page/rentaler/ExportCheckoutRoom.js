          ...
          <div className="container-fluid p-0">
            <div className="card">
              <div className="card-header">
                <h5 className="card-title">Xuất hóa đơn trả Phòng trọ</h5>
              </div>
              <div className="card-body">
                <form onSubmit={handleSubmit}>
                  ...
                  <button
                    type="submit"
                    onClick={() => handleExport(contractData.room.id)}
...
//   const currentMonth = parseInt(new Date().getMonth());
  ...
  const start = new Date(contractData.createdAt);
  const end = new Date(contractData.deadlineContract);
  const timeDiff = Math.abs(end.getTime() - start.getTime());
  const numOfDays = Math.ceil(timeDiff / (1000 * 3600 * 24));

  const result = numOfDays * pricePerMonth + priceRequest;
